...
Two consecutive Hallmark outputs in the same folder use the same macrostructure. The first emitted left-margin numbered labels + huge serif + asymmetric spans (Specimen); the second did exactly the same. The page looks redesigned only because copy changed.
...
**Fix.** Pick one library per folder. Lucide is the default for SaaS, Phosphor when you need weight variants, Heroicons for Tailwind/shadcn folders. See [`assets.md`](assets.md) for the canon.
