...
- `candidateIndex` is the provided 1-based index of the candidate the decision answers.
...
- Return exactly one decision per candidate in the same order, each with its own `candidateIndex`. Never add, omit, reorder, or merge candidates.
