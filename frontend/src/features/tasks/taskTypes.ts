// 이동 응답에만 필요한 타입 참조다. C안(계획 날짜를 task 컬럼으로)으로 가면 이 import가 사라진다.
...
/**
 * 할 일의 분류 정보를 한 번에 바꾼다. 폴더·중요·즉시는 task의 속성이고 계획 날짜는 별도 테이블이지만,
 * 사용자에게는 모달 하나의 저장이라 서버가 한 트랜잭션으로 처리한다.
 * folderId는 null이 "미분류"를 뜻해 생략과 구분되지 않으므로 세 값을 항상 보낸다.
 * 계획 항목이 없는 화면(폴더 목록·매트릭스)에서는 plan을 보내지 않는다.
 * TODO(task-owns-plan-date): 계획 날짜가 task 컬럼이 되면 plan 객체는 planDate 한 필드로 줄어든다.
...
export interface UpdateTaskInfoRequest {
  ...
  urgent: boolean
  plan?: {
    /** 계획 항목 id. 이미 담긴 항목을 옮길 때만 보낸다. 없으면 그 날짜의 계획에 새로 담는다. */
    itemId?: number
    date: string
  }
}
...
export interface UpdateTaskInfoResponse {
  task: TaskResponse
  /** 이동으로 바뀐 날짜들의 계획. 계획을 옮기지 않았으면 빈 배열이다. */
  plans: DailyPlan[]
}
...
/** store가 캐시하는 task의 최소 모양. 계획 응답처럼 createdAt·updatedAt이 없는 곳에서도 채울 수 있다. */
