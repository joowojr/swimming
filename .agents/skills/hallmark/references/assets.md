...
| 5 | **Local `public/placeholder-<type>.{jpg,svg}`** | Self-contained folders with no third-party deps. Single neutral grey-block SVG checked into the repo. |
...
| **[Heroicons](https://heroicons.com)** | `heroicons.com` | ~300 | Tailwind / shadcn folders. Tightly curated, opinionated. |
...
1. **Pick one library per folder.** Mixing Material + Heroicons + Lucide on the same page is the icon-set tell. The skill's audit verb catches this.
...
- **Material Icons in a non-Material folder** — gives a Google look that doesn't match anything else.
...
3. **One library per folder.** Mixing Storyset + Humaaans + unDraw = visual chaos. Pick one, stick to it.
