...
2. **Tailwind v4 `@theme`** — for folders on Tailwind. Emitted into `design.md`'s Exports section on multi-page folders.
3. **DTCG `tokens.json`** — for folders using a token pipeline (Style Dictionary, Token Studio, Cobalt). Emitted into `design.md`.
4. **shadcn/ui CSS variables** — for folders using shadcn/ui's component library. Emitted into `design.md`.

The output rule: `tokens.css` is always written. The other three live inline in `design.md` so the user copies whichever they need into a new folder. **No new verb.** This is a side effect of every build.
...
Or, if the folder uses a CSS bundler / framework that doesn't honour bare `@import`, the folder's existing entry-point imports `tokens.css` before the page CSS that consumes it (keep it among the other top-of-file `@import`s, never above or in place of `@import "tailwindcss"`). The page CSS still references tokens by name, never by raw value.
...
W3C Design Tokens Community Group format. For folders using Style Dictionary, Cobalt, or Token Studio. Path-based; OKLCH stays as a string value; `$type` is required.
...
1. **Always** write `tokens.css` next to the page CSS (or in the folder root for multi-file folders). Format 1.
2. **On `design.md`-managed folders** (multi-page), embed all four formats inline in `design.md`'s Exports section. The user copies whichever they need.
3. **On Tailwind folders** (detected at pre-flight), additionally surface the Tailwind `@theme` block in the build output so the user knows where to paste it (typically into `app/globals.css` or the equivalent).
4. **Merge into an existing entry stylesheet; never overwrite it.** When the target folder already has `app/globals.css` (or `src/index.css`, `src/styles/global.css`): keep its existing `@import "tailwindcss"` / `@tailwind base|components|utilities` directives exactly as they are, append Hallmark's `:root` tokens and base rules *after* them, and keep any `@import "tokens.css"` at the very top of the file (CSS parses `@import` only before other rules, so a misplaced one is silently dropped and your tokens vanish). If the folder already defines brand tokens (`--background`, `--foreground`, a Tailwind `@theme`), map Hallmark's roles onto those names rather than adding a parallel set, or scope Hallmark's tokens under a wrapper class. Replace the file outright only when the user has asked for a full takeover.

Don't blanket-emit tokens.json or shadcn variables on single-page folders — the user can copy them out of `design.md` if they upgrade to a multi-page system.
