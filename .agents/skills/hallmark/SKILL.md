...
**Implementation safety rail.** Hallmark is a design skill, not a license to bulldoze a codebase. In any existing folder:
...
Before entering the full Design flow, **check scope**. If any of these fire, run the Component-scope flow instead — most day-to-day dev requests are component-shaped, not page-shaped, and the page-level apparatus (macrostructure, hero enrichment, footer archetype, folder memory) is wrong for them.
...
- **Step 0 · Pre-flight scan** — same. Read existing tokens, fonts, framework, microinteraction stance. A button on a Geist-bodied Tailwind folder must adopt those tokens, not invent new ones.
...
1. **The component artifact** — a single self-contained file matching the folder's conventions:
...
If the folder already has code — a `package.json`, a `tailwind.config.*`, an `index.html`, any CSS — Hallmark should **read it before asking the user anything**. Stomping on an established palette or font stack is the difference between a skill the user keeps and a skill the user uninstalls.

**Six signal sources, scanned in order:**

0. **`design.md`** — at the folder root (or `DESIGN.md`). If present, this is the **locked design system for the folder** — written by a previous `hallmark redesign` run on the whole app, or by hand. **Read it first; it overrides everything else.** Subsequent picks (genre, theme, type, motion) defer to it. The diversification rule is *inverted* on `design.md`-managed folders: pages must share the system, not differ from each other. See [`verbs/redesign.md`](references/verbs/redesign.md) § Multi-page flow for how the file is produced and amended.
...
3. **Microinteraction stance** — `package.json` dependencies for `framer-motion`, `gsap`, `motion`, `lenis`, `lottie-react`, `@react-spring/*`, `auto-animate`. Any one of those = "motion-on" folder. None = "motion-cut" folder.
...
- **`design.md` found** → emit *"`design.md` detected at folder root — this is a system-managed folder. Reading the locked design system; subsequent picks defer to it."* Then read the file in full and use it as the source of truth for genre / theme / typography / spacing / motion / CTA voice. Skip Step 1's catalog/custom dispatch; the system is already chosen. Proceed to macrostructure pick (Step 2) within the family `design.md` allows for this page's type.
...
- **No signals found** (vanilla HTML folder, empty repo, scratch directory) → silent. One line only: *"No pre-flight signals — proceeding with full Hallmark stack."*
...
- **Empty folder** (no `package.json`, no `index.html`) → silent.
- **The user said "ignore the existing folder"** → skip pre-flight entirely; emit *"Pre-flight skipped at user request."* and proceed to Step 1.

**Two more sample outputs** for the model to imitate:

*Vanilla HTML folder, motion-cut:*
...
> *Pre-flight findings: Astro 5 (astro.config.mjs L1) · Tailwind v4 with @theme inline tokens (src/styles/global.css L3) · `tokens.json` at folder root (DTCG format, 12 colour tokens, 6 font tokens). No motion library detected.*
...
The pre-flight block is the user's accountability line: *"here's what I noticed about your folder before I touched anything."* Skipping it is the fastest way to lose the user's trust.
...
2. If you have produced any other Hallmark output for this user in this sessionEntity, your pick must be a different macrostructure than the last one.
...
If the file doesn't exist, this is the first Hallmark run for this folder — no constraint, but **you'll create the file in Step 6**.

If the folder has a CSS stamp but no `log.json`, infer one entry from the stamp and proceed.
...
- **First-time** (no `log.json`, fresh folder): no rotation block at all — just the macrostructure pick. *"This is the first Hallmark run for this folder. Picking Long Document — fits the Coffeebox brief's editorial tone."*
- **Mature folder** (5+ entries in `log.json`): the format above — frequency count, exclusion list, pick.
...
- [`export-formats.md`](references/export-formats.md) — load at Step 6 only when the folder warrants multi-format exports (i.e. has a `design.md`). Single-page builds emit `tokens.css` from the in-memory token state and don't need this file.
...
Skip the CTA line when (a) the build is component-scope, or (b) `design.md` already exists at the folder root (the system is already locked). See [`design-md.md`](references/design-md.md) for the full opt-in flow.
...
- **Stamp the output.** The first non-empty line of the produced CSS file (or the top of `<style>` if inline) MUST be a comment of the form: `/* Hallmark · macrostructure: <name> · tone: <tone> · anchor hue: <hue> */`. This stamp is the durable record of what you chose. The next time Hallmark runs in this folder, it reads the stamp and picks a *different* macrostructure. **For custom themes**, the stamp also carries the vibe, paper + accent OKLCH values, the chosen display + body fonts, and the three diversification axes — the full multi-line format is in [`custom-theme.md`](references/custom-theme.md) § E. **For studied-DNA builds** (Step 2.6 Condition 0 routed here from a `study` diagnosis), the stamp's `theme:` field is `studied-DNA (source: <URL or "image">)` followed by the paper OKLCH, accent OKLCH, and display + body fonts pulled directly from the diagnosis — not a catalog theme name. Diversification stays suspended for the run; the log entry below records `theme: studied-DNA` so Step 2.5 on the next run knows not to rotate against it.
- **Append to folder memory.** After you write the stamp, update (or create) `.hallmark/log.json` at the folder root. Append a new entry at the **front** of the array: `{ "date": "<YYYY-MM-DD>", "macrostructure": "<name>", "theme": "<name>", "enrichment": "<E# name or 'none'>", "brief": "<one-line summary>" }`. **Custom entries** also carry `"theme": "custom"` plus `"theme_axes": "<paper-band> / <display-style> / <accent-hue>"` and an optional `"vibe": "<4–8 words>"` — see [`custom-theme.md`](references/custom-theme.md) § F. Trim the file to the last 20 entries (rotate the oldest off). Create `.hallmark/` and the file if they don't exist; respect any existing `.gitignore` (the user may or may not want this committed). This file is what Step 2.5 reads on the next run.
- **Never clobber an existing global stylesheet.** When the folder already ships an entry stylesheet (`app/globals.css`, `src/index.css`, `src/styles/global.css`), it is **append-only**: keep its `@tailwind` / `@import "tailwindcss"` directives in place, add Hallmark's `:root` block and base rules below them, keep any new `@import` at the very top above all rules, and reuse the folder's own token names (`--background`, `--foreground`, a Tailwind `@theme`) where they exist. Overwrite the file only if the user explicitly asks: silently removing a framework's CSS entry directives un-styles the entire app. See [`contract.md`](references/contract.md).
- **Always emit `tokens.css`.** After writing the page CSS, also write `tokens.css` at the folder root containing every `--color-*`, `--font-*`, `--space-*`, `--text-*`, `--ease-*`, `--dur-*`, `--rule-*`, and `--radius-*` token used in the build. The page CSS imports `tokens.css` (or, on framework folders, the folder's existing entry-point includes it) — the page CSS must reference tokens by name, never inline raw values. Even single-page builds get a `tokens.css`. This is what makes the design system portable to the next folder. Load [`export-formats.md`](references/export-formats.md) at this point only when the folder warrants additional formats — see below.
- **Multi-format exports on `design.md` folders.** If a `design.md` exists at the folder root (a system-managed folder), append all four export formats — `tokens.css`, Tailwind v4 `@theme`, DTCG `tokens.json`, shadcn/ui CSS variables — into `design.md`'s `## Exports` section. Load [`export-formats.md`](references/export-formats.md) for the canonical mapping from Hallmark tokens to each format. Single-page folders skip this step (they get only `tokens.css`).
