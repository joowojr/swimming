...
class KnowledgeSourceControllerTest {
    ...
    void setUp() {
        ...
        listUseCase = mock(SourceListUseCase.class);

        mockMvc = MockMvcBuilders
                .standaloneSetup(new KnowledgeSourceController(collectUseCase, listUseCase))
    ...
    void collectsLinks() throws Exception {
        when(collectUseCase.collect(eq(1L), eq(10L), any(SourceCollectRequest.class)))
    ...
    void rejectsOtherUsersFolder() throws Exception {
        when(collectUseCase.collect(eq(1L), eq(99L), any(SourceCollectRequest.class)))
    ...
    void listsSources() throws Exception {
        when(listUseCase.list(1L, 10L, null, 20, null))
    ...
    void passesGetListQuery() throws Exception {
        when(listUseCase.list(1L, 10L, SourceProcessingStatus.PENDING, 5, "cursor-abc"))
        ...
        verify(listUseCase).list(1L, 10L, SourceProcessingStatus.PENDING, 5, "cursor-abc");
    }

    @Test
    @DisplayName("조건을 주지 않으면 최근 20개를 조회한다")
    void usesDefaultLimit() throws Exception {
        when(listUseCase.list(eq(1L), eq(10L), isNull(), eq(20), isNull()))
                .thenReturn(new CursorPage<>(List.of(), null, false));

        mockMvc.perform(get("/api/folders/10/knowledge/sources"))
                .andExpect(status().isOk());

        verify(listUseCase).list(1L, 10L, null, 20, null);
    }
