...
//            "gpt-4.1",
    ...
    @Test
    @DisplayName("세 모델의 실제 사용자 시나리오 분류 결과를 JSON 리포트로 생성한다")
    ...
    void generatesDigitalMarketingFreelancerEvaluationReport() throws Exception {
        generateReport(
                ...
                "복수 클라이언트의 SNS·퍼포먼스 광고·브랜드 런칭과 프리랜서 운영 업무를 병행하는 30대 여성 디지털 마케팅 프리랜서",
                digitalMarketingFreelancerScenarios()
        );
    }
    ...
    private ScenarioResult evaluate(Scenario scenario, String model) {
    ...
    private List<Scenario> digitalMarketingFreelancerScenarios() {
        ProjectContext lightFit = project(
                101L,
                "라이트핏 SNS 운영",
                "여성 운동복 브랜드의 인스타그램 콘텐츠, 릴스와 댓글 이벤트 운영"
        );
        ProjectContext momoSkin = project(
                102L,
                "모모스킨 퍼포먼스 광고",
                "스킨케어 브랜드의 Meta·Google 광고, 전환 추적과 ROAS 개선"
        );
        ProjectContext breezeCafe = project(
                103L,
                "브리즈카페 런칭",
                "신규 카페의 인스타그램, 블로그, 체험단과 오픈 캠페인 운영"
        );
        ProjectContext freelanceOperations = project(
                104L,
                "프리랜서 운영",
                "클라이언트 계약, 견적서, 인보이스, 세금계산서와 포트폴리오 관리"
        );

        return List.of(
                scenario(
                        "marketer-mixed-client-personal",
                        "두 클라이언트 업무와 개인 용무가 섞인 메모",
                        "라이트핏과 모모스킨 업무를 각각 분류하고 세탁소 용무는 다듬은 미분류 항목으로 남기는지 평가",
                        """
                                라핏 이번주 릴스 3개 캡션 아직 덜씀 해시태그도 다시 보기
                                모모 메타 여름세일 광고 UTM 이름 지난거랑 겹치는지 체크
                                아 겨울코트 세탁소 맡긴거 토요일에 찾아야함
                                """,
                        List.of(lightFit, momoSkin, breezeCafe, freelanceOperations),
                        List.of(
                                task(1011L, lightFit, "주간 릴스 콘텐츠 캘린더 작성"),
                                task(1021L, momoSkin, "Meta 여름 세일 캠페인 세팅")
                        )
                ),
                scenario(
                        "marketer-split-three-projects",
                        "한 문장에 섞인 세 업무 영역 분리",
                        "브리즈카페 체험단, 모모스킨 광고 문구, 프리랜서 세금계산서 업무를 각각 분리하는지 평가",
                        "브리즈 체험단 후보 20명 추리고 모모 리타겟 광고 문구도 두개 더 써야되고 이번달 두 클라 세금계산서 발행도 안함",
                        List.of(lightFit, momoSkin, breezeCafe, freelanceOperations),
                        List.of(
                                task(1031L, breezeCafe, "인스타그램 체험단 모집"),
                                task(1022L, momoSkin, "리타겟 광고 소재 제작"),
                                task(1041L, freelanceOperations, "월별 세금계산서 발행")
                        )
                ),
                scenario(
                        "marketer-merge-campaign-report",
                        "흩어진 광고 리포트 작업 병합",
                        "캠페인 데이터·ROAS·CTR 확인을 하나의 모모스킨 리포트 Task로 병합하고 장보기는 미분류하는지 평가",
                        """
                                모모 7월 캠페인 리포트 뽑기
                                메타 광고비랑 ROAS 지난달 비교도 넣어야댐
                                고양이 사료 주문
                                아까 그 리포트 소재별 CTR 낮은것도 표시
                                """,
                        List.of(momoSkin, lightFit, freelanceOperations),
                        List.of(task(1023L, momoSkin, "월간 광고 성과 리포트 작성"))
                ),
                scenario(
                        "marketer-ambiguous-instagram-event",
                        "두 브랜드 사이에서 모호한 인스타그램 이벤트",
                        "어느 클라이언트인지 단서가 없는 이벤트 업무를 임의 배정하지 않고 다듬은 미분류 항목으로 남기는지 평가",
                        "인스타 댓글 이벤트 당첨자 뽑고 디엠 보내야되는데 이거 어느 클라건지 메모를 안해놨네",
                        List.of(lightFit, breezeCafe),
                        List.of(
                                task(1012L, lightFit, "인스타그램 댓글 이벤트 운영"),
                                task(1032L, breezeCafe, "오픈 기념 인스타그램 이벤트 운영")
                        )
                ),
                scenario(
                        "marketer-client-nicknames",
                        "클라이언트 별칭으로 업무 분류",
                        "라핏과 브카라는 러프한 별칭을 각각 올바른 브랜드 Project로 연결하는지 평가",
                        "라핏 릴스 자막 폰트 가이드랑 안맞는거 수정 / 브카 네이버 지도 오픈시간 사진이랑 다른거 고치기",
                        List.of(lightFit, breezeCafe, momoSkin),
                        List.of(
                                task(1013L, lightFit, "릴스 편집 가이드 정리"),
                                task(1033L, breezeCafe, "네이버 플레이스 정보 등록")
                        )
                ),
                scenario(
                        "marketer-unrelated-only-project",
                        "프리랜서 운영과 무관한 개인 일정",
                        "Project가 프리랜서 운영 하나뿐이어도 치과 예약을 강제 배정하지 않는지 평가",
                        "치과 스케일링 예약 다음주 수요일 되는지 전화해봐야겠다",
                        List.of(freelanceOperations),
                        List.of(task(1042L, freelanceOperations, "3분기 부가세 자료 정리"))
                ),
                scenario(
                        "marketer-no-projects",
                        "Project 없이 적은 업무와 개인 메모",
                        "Project가 없을 때 클라이언트 확인과 건강검진 예약을 각각 다듬은 미분류 항목으로 보존하는지 평가",
                        "이번주 콘텐츠 컨펌 누구한테 받아야하는지 확인\n건강검진 예약 날짜 다시 잡기",
                        List.of(),
                        List.of()
                ),
                scenario(
                        ...
                        "오타와 구어체가 섞인 체험단 메모",
                        "오타가 있는 원문은 sourceText에 보존하고 제목은 브리즈카페 체험단 후속 행동으로 다듬는지 평가",
                        "브리즈 인플루언서 리스트 컨텍 안한사람 다시 디엠보내고 답온거 시트 체쿠",
                        List.of(breezeCafe, lightFit, freelanceOperations),
                        List.of(task(1034L, breezeCafe, "체험단 인플루언서 컨택"))
                ),
                scenario(
                        "marketer-existing-task-context",
                        "기존 광고 Task의 고유 용어로 Project 분류",
                        "Project 이름 없이 CAPI·구매 이벤트 맥락을 기존 Task 묶음으로 읽어 모모스킨에 분류하는지 평가",
                        "CAPI 구매 이벤트 브라우저랑 서버 둘다 잡혀서 두번 찍히는거 확인해야함",
                        List.of(momoSkin, lightFit),
                        List.of(
                                task(1025L, momoSkin, "Meta Pixel 및 CAPI 전환 추적 설정"),
                                task(1026L, momoSkin, "구매 이벤트 중복 제거 검증"),
                                task(1014L, lightFit, "릴스 조회수 리포트 작성")
                        )
                ),
                scenario(
                        "marketer-actions-vs-thoughts",
                        "프리랜서 업무와 감상·아이디어 조각 혼합",
                        "명시된 인보이스·성과표 업무만 Task로 만들고 피로감·주제 고민·막연한 생각은 다듬은 미분류 항목으로 남기는지 평가",
                        """
                                요즘 클라 피드백 늦게 오면 괜히 하루종일 신경쓰임
                                이번달 인보이스 세곳 보내기
                                브리즈 블로그 키워드 아직 감 안옴 그냥 데이트코스?
                                라핏 8월 릴스 성과표 업데이트
                                다음달에 뭔가 새 서비스 만들어볼까 아직 모르겠음
                                """,
                        List.of(lightFit, momoSkin, breezeCafe, freelanceOperations),
                        List.of(
                                task(1043L, freelanceOperations, "월별 클라이언트 인보이스 발송"),
                                task(1035L, breezeCafe, "블로그 검색 키워드 기획"),
                                task(1015L, lightFit, "월간 릴스 성과 리포트 작성")
                        )
                )
        );
    }
    ...
    private static Scenario scenario(
        ...
        return new Scenario(id, name, evaluationCriteria, input(memo, projects, tasks));
    }
