...
- The codebase has multiple route files (`app/page.tsx`, `app/about/page.tsx`, `app/pricing/page.tsx`, etc.) and the user pointed at the folder root.
...
### 2. Produce `design.md` at the folder root

Write a single file at the folder root: `design.md` (or `DESIGN.md` — match the folder's existing case convention). This file is the **one source of truth** every subsequent page redesign reads. Format:
...
Drop-in formats for re-using this design system in other folders.
...
- **Read `design.md` first.** It is now the rule of the folder; the per-build references in [`references/`](../) defer to it. Where `design.md` and the references conflict, `design.md` wins.
