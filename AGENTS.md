...
- Backend, database, API contracts, backend tests, infrastructure, and folder documentation may be implemented through the normal implementation workflow.
- Codex may create, complete, or modify frontend components, pages, hooks, stores, types, API integration, validation, routing, and feature logic within the requested Feature Spec.
- Frontend implementation follows the normal implementation workflow: confirm the API contract, present the affected files and plan, wait for approval, implement, and verify the relevant behavior.
...
- Lean on the dependencies already in the folder before writing your own implementation or adding packages. Do not assume a library lacks a capability without checking its documentation and types.
...
- 백엔드 패키지는 `com.swimming.backend` 아래 도메인 단위로 나눈다 (auth, health, user, folder, task, plan, session, group, place, stats, note, common).
...
- Controller는 같은 도메인의 UseCase만 호출한다. 작은 단일 동작에도 Controller → Service 직접 호출 예외를 두지 않는다.
- 수정 방식은 단건과 여러 행을 구분한다. 단건 수정은 소유 Entity를 조회한 뒤 필요한 도메인 인스턴스 메서드 또는 Entity 변경 메서드를 호출하고 변경 감지로 저장한다. 단건 필드 수정마다 별도 API·Service 메서드를 둔다.
- 여러 행에 같은 상태·완료값을 적용하거나 여러 행을 삭제·연결 해제할 때는 `@Modifying` JPQL을 사용한다. 사용자·부모 ID 조건을 포함하고 반환 영향 행 수를 검증한다. 벌크 JPQL에는 `flushAutomatically = true`, `clearAutomatically = true`를 적용한다.
- 벌크 JPQL은 도메인 인스턴스 메서드와 JPA Auditing을 거치지 않으므로 `updatedAt` 등 필요한 값을 쿼리에 명시한다. 단건 수정에 벌크 쿼리를 사용해 상태 검증과 변경 감지를 우회하지 않는다.
- 도메인 상태 전이가 필요한 유스케이스는 UseCase가 도메인 객체를 조회·조율하고, 상태 전이·현재 상태 검증·파생값 계산은 도메인 인스턴스 메서드가 담당하게 한다. Service는 결과를 영속 Entity에 반영하며, 단건 필드는 변경 감지로 저장한다. 상태 전이에 따른 여러 하위 행 변경은 별도 `@Modifying` JPQL로 분리하고 영향 행 수를 검증한다.
- 도메인 간 협업 규칙: 다른 도메인은 **Service를 통해서만** 호출하고(남의 Repository 직접 주입 금지), **JPA 엔티티를 직접 전달하지 않는다**. Service는 필요에 따라 DTO 또는 순수 도메인 객체를 반환할 수 있다. Service 호출 방향은 auth → user, folder → task, plan → folder·task, session → user·place·plan·task, note → folder·session·task, group → user·place·session·task, stats → folder·task·session·place로 유지하며 health는 독립시킨다.
- 다른 도메인의 JPA Entity 타입 참조는 Entity 연관 매핑과 자기 Repository에 저장할 reference 조립에만 허용한다. Entity를 Service 입출력으로 전달하거나 상대 Repository를 직접 호출하지 않는다.
...
- 스키마를 바꿔야 하면 새 Flyway 마이그레이션(`backend/src/main/resources/db/migration/V2__...`)을 추가하고 `docs/erd.mmd`를 함께 갱신한다.
- 한 슬라이스가 끝나면 커밋한다.
...
- `main`은 항상 배포된 상태를 가리킨다. 여기에 직접 커밋하지 않는다.
- 기본 브랜치는 `develop`이다. 작업 브랜치는 develop에서 분기하고 develop로 PR을 연다.
- 브랜치 이름은 `feature/<번호>-<주제>` 또는 `refactor/<주제>`를 쓴다 (예: `feature/13-link-graph`).
- develop → main PR은 배포 시점에만 연다.
